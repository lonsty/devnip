...
const CSS_COLORS = {
  ...
  darkslategray:[47,79,79],darkturquoise:[0,206,209],darkviolet:[148,0,211],deeppink:[255,20,147],
  deepskyblue:[0,191,255],dimgray:[105,105,105],dodgerblue:[30,144,255],firebrick:[178,34,34],
  floralwhite:[255,250,240],forestgreen:[34,139,34],fuchsia:[255,0,255],gainsboro:[220,220,220],
  ghostwhite:[248,248,255],gold:[255,215,0],goldenrod:[218,165,32],gray:[128,128,128],
  green:[0,128,0],greenyellow:[173,255,47],honeydew:[240,255,240],hotpink:[255,105,180],
  ...
  lightgray:[211,211,211],lightgreen:[144,238,144],lightpink:[255,182,193],lightsalmon:[255,160,122],
...
export const ColorTool = {
  parse(input) {
    ...
    // CSS color name
    if (CSS_COLORS[s]) {
      const [r, g, b] = CSS_COLORS[s];
      return { success: true, data: { r, g, b, a: 1 } };
    }
    // HEX
    ...
    if (m) {
      ...
      if (hex.length === 3) {
        ...
        return { success: false, error: 'Invalid HEX color value' };
      }
      return { success: true, data: { r, g, b, a: Math.round(a * 100) / 100 } };
    }
    // rgb/rgba
    m = s.match(/rgba?\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*(?:,\s*([\d.]+))?\s*\)/);
    if (m) {
      return { success: true, data: { r: +m[1], g: +m[2], b: +m[3], a: m[4] !== undefined ? +m[4] : 1 } };
    }
    // hsl/hsla
    m = s.match(/hsla?\(\s*([\d.]+)\s*,\s*([\d.]+)%?\s*,\s*([\d.]+)%?\s*(?:,\s*([\d.]+))?\s*\)/);
    if (m) {
      const rgba = this._hslToRgb(+m[1], +m[2], +m[3]);
      rgba.a = m[4] !== undefined ? +m[4] : 1;
      return { success: true, data: rgba };
    }
  ...
  toAll(rgba) {
    ...
    return { hex: hexA.toUpperCase(), rgb, hsl: hslStr };
  },
