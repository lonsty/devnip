// Hash 计算工具（SHA 系列 + MD5）
export const HashTool = {
  async compute(text, algorithm = 'SHA-256') {
    try {
      const data = new TextEncoder().encode(text);
      if (algorithm === 'MD5') {
        return { success: true, data: md5(data) };
      }
      ...
      return { success: false, error: `Hash computation failed: ${e.message}` };
    }
  },

  async computeAll(text) {
    const algorithms = ['MD5', 'SHA-1', 'SHA-256', 'SHA-512'];
    const results = {};
    for (const alg of algorithms) {
      const r = await this.compute(text, alg);
      results[alg] = r.success ? r.data : r.error;
    }
...
function md5(inputBytes) {
  ...
  const padded = new Uint8Array(((msgLen + 8 >> 6) + 1) * 64);
